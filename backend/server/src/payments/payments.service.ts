...
export class PaymentsService implements OnModuleInit, OnModuleDestroy {
  ...
  async createMomoPayment(
    ...
    this.logger.log(`[PaymentsService ${ts()}] [CREATE_MOMO_PAYMENT_START] Dto: ${JSON.stringify(createDto)}`);
    ...
    this.logger.log(`[PaymentsService ${ts()}] [CREATE_MOMO_PAYMENT_LOCK_SEATS] Attempting to lock seats=${seats.join(',')} for showtime=${showtime_id}`);
    await this.lockSeatsOrThrow(showtime_id, seats);

    let savedBooking: Booking & { _id: any };
    try {
      savedBooking = await newBooking.save();
      this.logger.log(`[PaymentsService ${ts()}] [CREATE_MOMO_PAYMENT_BOOKING_SAVED] Booking saved. ID=${savedBooking._id.toString()}, momoOrderId=${momoOrderId}`);
    } catch (error) {
      this.logger.error(`[PaymentsService ${ts()}] [CREATE_MOMO_PAYMENT_SAVE_ERROR] Error saving booking, releasing seats. Error: ${error.message}`);
      await this.seatReservation.releaseSeats({ showtime: showtime_id, seats });
      throw error;
    }
    const bookingId = savedBooking._id.toString();

    this.logger.log(`[PaymentsService ${ts()}] [CREATE_MOMO_PAYMENT_LOCK_PENDING] Syncing seats to Socket.IO. showtime=${showtime_id}, seats=${seats.join(',')}`);
    ...
    this.logger.log(`[PaymentsService ${ts()}] [CREATE_MOMO_PAYMENT_START_CHECKOUT] Starting MoMo Checkout for bookingId=${bookingId}, orderId=${momoOrderId}`);
  ...
  async retryPayment(
    ...
    const ts = () => new Date().toISOString();
    this.logger.log(`[PaymentsService ${ts()}] [RETRY_PAYMENT_START] bookingId=${bookingId}, userId=${userId}`);
    ...
    if (!booking) {
      this.logger.warn(`[PaymentsService ${ts()}] [RETRY_PAYMENT_NOT_FOUND] Pending booking not found. ID=${bookingId}, userId=${userId}`);
      throw new BadRequestException('Đơn hàng không tồn tại hoặc đã được xử lý');
    }

    const createdAt = (booking as any).createdAt;
    if (createdAt && Date.now() - new Date(createdAt).getTime() > BOOKING_TTL_MS) {
      this.logger.warn(`[PaymentsService ${ts()}] [RETRY_PAYMENT_EXPIRED] Booking has expired. ID=${bookingId}, createdAt=${createdAt}`);
    ...
    this.logger.log(`[PaymentsService ${ts()}] [RETRY_PAYMENT_NEW_ORDER_ID] Generated new momoOrderId=${newMomoOrderId} for bookingId=${bookingId}`);
    await this.bookingModel.findByIdAndUpdate(bookingId, {
  ...
  async cancelPendingBooking(
    ...
    const ts = () => new Date().toISOString();
    this.logger.log(`[PaymentsService ${ts()}] [CANCEL_PENDING_BOOKING_START] bookingId=${bookingId}, userId=${userId}`);
    const booking = await this.bookingModel.findOne({
      _id: bookingId,
      user: userId,
      status: 'pending',
    ...
    if (!booking) {
      this.logger.warn(`[PaymentsService ${ts()}] [CANCEL_PENDING_BOOKING_NOT_FOUND] Pending booking not found for cancel. ID=${bookingId}, userId=${userId}`);
      throw new BadRequestException('Đơn hàng không tồn tại hoặc đã được xử lý');
    }

    this.logger.log(`[PaymentsService ${ts()}] [CANCEL_PENDING_BOOKING_RELEASE] Releasing seats for booking=${bookingId}`);
    await this.seatReservation.releaseSeats(booking);
    await this.bookingModel.findByIdAndUpdate(bookingId, {
      status: 'cancelled',
    ...
    this.logger.log(`[PaymentsService ${ts()}] [CANCEL_PENDING_BOOKING_SUCCESS] Successfully cancelled booking=${bookingId}`);
